...
public class LoginInterceptor extends HandlerInterceptorAdapter{
	//1. 인터셉터 클래스는 스프링에서 제공되는 HandlerInterceptor클래스를 상속받음
	//2. 3가지 메서드를 오버라이딩 해서 사용합니다

	
	//preHandler는 컨트롤러를 실행하기 전에 요청을 가로챔
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if( user_id == null) {//인증값이 없음
			...
			return false;//핸들러메서드 실행 후에 컨트롤러를 수행하지 않음
		}else {//로그인된 유저
			return true;//핸들러메서드 실행 후에 컨트롤러를 수행
		}
	...
	//viewResolver처리 후, 화면으로 가기 전에 요청을 가로챕니다
	@Override

<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>                            
							...
							<tr>
								...
								<td>${vo.writer }</td>
								<td><fmt:formatDate value="${vo.regdate }" pattern="yy년 MM월 dd일 hh시mm분"/></td>
								<td><fmt:formatDate value="${vo.updatedate }" pattern="yy년 MM월 dd일 hh시mm분"/></td>
								<!-- 
		   ...
		   <%--  <form>
                    ...
                    <ul class="pagination pagination-sm">
                        ...
                        <c:forEach var="num" begin="${pageVO.startPage}" end="${pageVO.endPage}">                        
                        	...
                        	<a href="freeList?pageNum=${num}&amount=${pageVO.amount}">${num}</a></li>
                        </c:forEach>
		    ...
		    <form action="freeList" name="pageForm">
                    ...
                    <!-- 폼형식으로 보내는데 숨겨서 보낼 값 hidden으로 표시  -->
	...
	<script>
		...
		//3. 검색폼과 페이지폼이 동일한 값을가지고 hidden으로 넘어가도록 처리
		//4. sql문을 동적쿼리로 변경
		//페이저
		...
		pagination.onclick = function(){
			...
			var pageNum = event.target.dataset.page; //클릭하타겟의 데이터셋값.
	...
	<script>
		window.onload=function(){
			...
			//변견된 기록정보는 history.State 객체를 통해서 확인이 가능합니다
